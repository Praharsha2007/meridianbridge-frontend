...
const FOLLOWER_RANGES = ['1K – 20K ', '20K – 100K ', '100K – 250K ', '250K – 500K', '500K – 750K', '750K - 1M ', '1M+'];
    ...
    <div style={{ position: 'relative', marginTop: 10 }}>
      ...
      {open && (
        <div style={{
          ...
          borderRadius: 12, overflow: 'auto', maxHeight: 220
        }}>
...
const BUDGET_PRESETS = ['₹10,000 – ₹25,000', '₹25,000 – ₹50,000', '₹50,000 – ₹75,000', '₹1,00,000 – ₹5,00,000', '₹5,00,000+', 'Open to discussion'];
...
export default function Apply() {
  ...
  const res = await fetch(`${API_URL}/influencer/me`, 
  ...
  try {
    ...
    if (screen === "brand") {
      response = await fetch(`${API_URL}/brand/profile`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
  name: brandData.name,
  company_name: brandData.company,
  phone: brandData.phone,

  // 🔥 ADD THESE
  goal: brandData.goal === "Others"
    ? brandData.goalOther
    : brandData.goal,

  platform: brandData.platform,
  languages: brandData.languages,
  budget: brandData.budgetFixed,
  follower_range: brandData.followerRange
})
      });
    }

    if (screen === "creator") {
      response = await fetch(`${API_URL}/influencer/apply`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          ...
          profile_link: creatorData.handleLink,
          phone: creatorData.phone,
          followers: Number(creatorData.followers),
          avg_views: Number(creatorData.avgViews),
          genres: creatorData.genres,
          languages: creatorData.languages,
          experience: creatorData.collabExp,
          fee: creatorData.fee,
          categories: creatorData.brandCategories,
          long_term: creatorData.longTerm === "Yes"
        })
      });
    }
